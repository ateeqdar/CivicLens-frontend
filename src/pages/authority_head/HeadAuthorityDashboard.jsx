          ...
          <div className="mb-10 relative">
            ...
            <div className="relative flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div>
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-900 text-white text-[10px] font-black uppercase tracking-[0.2em] mb-4">
                  ...
                  <h1 className="text-4xl font-black text-slate-900 mb-2 tracking-tight">Executive Command</h1>
                  <p className="text-lg text-slate-500 font-medium">Global oversight of all municipal departments and civic infrastructure.</p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
              {stats.map((stat) => (
                <div 
                  key={stat.label}
                  className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/40 relative overflow-hidden group hover:-translate-y-1 transition-all"
                >
                  <div className={`absolute top-0 right-0 w-32 h-32 ${stat.bg} opacity-10 rounded-full -mr-16 -mt-16 transition-transform group-hover:scale-110 duration-500`} />
                  <div className="relative">
                    <div className="flex items-center justify-between mb-6">
                      <div className={`${stat.bg} ${stat.color} p-4 rounded-2xl shadow-lg shadow-current/5`}>
                        <stat.icon className="h-7 w-7" />
                      </div>
                      <div className="flex flex-col items-end">
                        <span className="flex items-center text-xs font-black text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-100">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 mb-12">
              ...
              <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/40 p-10">
                <div className="flex items-center justify-between mb-10">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h2 className="text-2xl font-black text-slate-900">Department Metrics</h2>
                      <p className="text-sm text-slate-400 font-bold uppercase tracking-widest">Efficiency Benchmarking</p>
                    </div>
                  </div>
                  <button className="bg-slate-50 hover:bg-slate-100 text-slate-600 px-6 py-2.5 rounded-xl text-sm font-black transition-all">
                    Export Data
                  </button>
                </div>
                  ...
                  {deptStats.map((dept) => (
                    ...
                    >
                      <div className="flex justify-between items-end mb-3">
                        <div>
                          <p className="text-lg font-black text-slate-900 group-hover:text-primary-600 transition-colors">{dept.name}</p>
                          <div className="flex items-center gap-3 mt-1">
                            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{dept.count} Active Tasks</span>
                            <div className="w-1 h-1 rounded-full bg-slate-200" />
                            <span className="text-xs font-bold text-primary-500 uppercase tracking-widest">{dept.resolved} Resolved</span>
                          </div>
                        </div>
              ...
              <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl shadow-slate-900/20">
                <div className="relative z-10 flex flex-col h-full">
                  <div className="mb-8">
                    <div className="bg-white/10 w-14 h-14 rounded-2xl flex items-center justify-center mb-6 backdrop-blur-md border border-white/10">
                      <MapIcon className="h-7 w-7 text-primary-400" />
                    </div>
                    <h2 className="text-2xl font-black mb-3 tracking-tight">Geospatial Insights</h2>
                  ...
                  <div className="space-y-4 mb-10 flex-grow">
                    {[
                      ...
                      <div key={zone.name} className="bg-white/5 backdrop-blur-xl rounded-2xl p-5 border border-white/5 hover:bg-white/10 transition-all cursor-pointer group">
                        <div className="flex justify-between items-center mb-3">
                          <span className="font-black text-slate-200">{zone.name}</span>
                          <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-lg ${zone.bgLight} ${zone.text}`}>
                            {zone.status}
                          </span>
                        </div>
                  ...
                  <button className="w-full bg-primary-600 text-white py-5 rounded-2xl font-black hover:bg-primary-500 transition-all shadow-xl shadow-primary-900/20 flex items-center justify-center gap-3 group">
                    Open Strategy Map
